...
== Apprenda Updates
We made the following changes to the base application to ease deployment to Apprenda:

1. Modify the pom.xml and Application.java files to support the deployment as a WAR file (http://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#build-tool-plugins-maven-packaging)
2. Added an application.properties file to provide an override to the default mongodb uri property. By default, the application will expect MongoDB to be running and accessible via localhost, which will not be the case in a cloud platform. Instead, we are using Apprenda's Add-On model to automatically wire-in the proper connection string. 
3. The final version is in the *complete* folder.

== Apprenda Requirements
1. Prior to the deployment of the application, you will need an instance of MongoDB available. 
2. If you wish to leverage the Apprenda Add-On model, simply deploy the following: https://github.com/apprenda/Apprenda-MongoDB
3. Make sure your platform has Java 1.8 available via the SOC. 

== Additional Notes
1. In order to build the application, simply run *mvn package*
2. You will need to have an available instance of MongoDB or the test run during the build process will fail. Provide the URI in the application.properties file. 
